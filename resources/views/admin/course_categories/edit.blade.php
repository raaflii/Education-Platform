...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Edit Kategori: {{ $category->name }}</h1>
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="relative">
                            @if($category->image)
                                <img src="{{ Storage::url($category->image) }}" alt="{{ $category->name }}" class="w-32 h-32 rounded-md object-cover">
                            @else
                        ...
                        <div>
                            <button type="button" onclick="document.getElementById('image').click()" 
                                ...
                                {{ $category->image ? 'Ganti Gambar' : 'Pilih Gambar' }}
                            </button>
                            @if($category->image)
                                <button type="button" onclick="document.getElementById('remove-image').value = '1'; document.getElementById('image-preview').classList.add('hidden')" 
                                    class="bg-red-100 text-red-700 px-4 py-2 rounded-md hover:bg-red-200 transition">
                                    Hapus Gambar
                                </button>
